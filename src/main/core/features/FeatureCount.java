...
public class FeatureCount {
	...
	/**
	 * true if this FeatureCount has at least one of each feature that other has
	 * 
	 * @param other
	 *            The FeatureCount that might be subsumed by this one.
	 * @return true if this FeatureCount's feature set subsumes the other's
	 *         feature set
	 */
	public boolean featureSetSubsumes(FeatureCount other) {
		for (int i = 0; i < featureCountArray.length; i++) {
			boolean thisHasFeature = this.featureCountArray[i] > 0;
			boolean otherHasFeature = other.featureCountArray[i] > 0;
			if (otherHasFeature && !thisHasFeature) {
				return false;
			}
		}
		return true;
