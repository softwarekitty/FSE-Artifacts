...
import main.core.edges.KruskalOutput;
import main.core.edges.ProportionOutput;
...
//			getMetagroupANOVAs(m_out_directory));
	...
	private static String getPairsFromTwo(File p2_out_directory,
		...
		File p2_out_compose_directory = null;//new File(IOUtil.dataPath + IOUtil.OUT +IOUtil.COMPOSITION + IOUtil.P2_PATH);
		...
		for (File p2File : p2_out_directory.listFiles()) {
			if (!p2File.isHidden()) {
				...
				List<AnswerColumn> inputColumns = null;//IOUtil.getColumns(IOUtil.getLines(p2_in_directory.getAbsolutePath() +"/" + inputFilename), "\t");
...
//						"\t");
				...
				ProportionOutput po = null;//new ProportionOutput(IOUtil.getLines(p2C.getAbsolutePath()), p2File.getName());
				sb.append(po.getCountText());

				WilcoxOutput wo = null;//new WilcoxOutput(IOUtil.getLines(p2File.getAbsolutePath()), p2File.getName());
	...
	private static String getTriples(File t_out_directory, File t_in_directory)
		...
		for (File tFile : t_out_directory.listFiles()) {
			if (!tFile.isHidden()) {
				...
				List<AnswerColumn> inputColumns = null;//IOUtil.getTripleColumns(IOUtil.getLines(t_in_directory.getAbsolutePath() +"/" + inputFilename), "\t");
				for (AnswerColumn ac : inputColumns) {
					sb.append(ac.getRegexCode() + "\t");
				}
				for (AnswerColumn ac : inputColumns) {
...
//						"\t");
				...
				KruskalOutput ko = new KruskalOutput(IOUtil.readLines(tFile.getAbsolutePath()), tFile.getName(), inputColumns, alpha);
				kOuts.add(ko);
	...
	private static String getPairsFromThree(File p3_out_directory,
		...
		File p3_out_compose_directory = null;//new File(IOUtil.dataPath + IOUtil.OUT +IOUtil.COMPOSITION + IOUtil.P3_PATH);
		...
		for (File p3File : p3_out_directory.listFiles()) {
			if (!p3File.isHidden()) {
				...
				List<AnswerColumn> inputColumns = null;//IOUtil.getColumns(IOUtil.getLines(p3_in_directory.getAbsolutePath() +"/" + inputFilename), "\t");
				for (AnswerColumn ac : inputColumns) {
					sb.append(ac.getRegexCode() + "\t");
				}
				for (AnswerColumn ac : inputColumns) {
...
//						"\t");
				...
				ProportionOutput po = null;//new ProportionOutput(IOUtil.getLines(p3C.getAbsolutePath()), p3File.getName());
				sb.append(po.getCountText());

				WilcoxOutput wo = null;//new WilcoxOutput(IOUtil.getLines(p3File.getAbsolutePath()), p3File.getName());
				sb.append(df3.format(wo.getWValue()) + "\t" +
	...
	private static String getMetagroupANOVAs(File m_out_directory)
			throws IOException {
		File m_out_compose_directory = null;//new File(IOUtil.dataPath + IOUtil.OUT +IOUtil.COMPOSITION + IOUtil.M_PATH);
		StringBuilder sb = new StringBuilder();
		...
		for (File mFile : m_out_directory.listFiles()) {
			if (!mFile.isHidden()) {
				sb.append(mFile.getName().subSequence(0, mFile.getName().indexOf(".")) +
					"\n");
				List<String> lines = IOUtil.readLines(mFile.getAbsolutePath());
				...
				List<String> cLines = IOUtil.readLines(m_out_compose_directory +
					"/" + mFile.getName());
				lineCounter = 0;
				while (lineCounter < nLinesToInclude) {
